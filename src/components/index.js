...
locationsData.forEach((loc) => {
    ...
    if (
        config.storeCoordinateGroups[config.storeCoordinateGroups.length - 1]
            .length === 24
    ) {
        // Start a new group
    ...
    config.storeCoordinateGroups[groupNumber].push(
        `${loc.geolocation.lng},${loc.geolocation.lat}`
    );
});
                                ...
                                {this.state.suggestions.map(
                                    (suggestion, key) => (
                                        <li
                                            ...
                                            onClick={this.updateWithSuggestion}
                                        >
                                            {suggestion.name}
                                        </li>
                                    )
                                ...
                                <tr key={key}>
                                    <td>
                                        <div className="location-header">
                                            <h3>
                                                <a
                                                    href={`https://www.hutsoninc.com/locations/${
                                                        location.handle
                                                    }/`}
                                                >
                                                    {location.title}
